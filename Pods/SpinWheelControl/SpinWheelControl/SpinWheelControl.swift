...
open class SpinWheelControl: UIControl {
    ...
    @objc func snapToNearestWedge() {
        ...
        let first = round(((currentRadians + (radiansPerWedge / 2))))
        let second = first + snappingPositionRadians
        let nearestWedge = Int(second / radiansPerWedge)
    ...
    @objc public func spin(velocityMultiplier: CGFloat = 0.85) {
